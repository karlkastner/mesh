...
% VECTORS v double
	...
	% skip 'SCALARS'
	% TODO check if TMP1 is SCALARS
	[tmp1, tmp2] = strtok(str);
	[fieldname, fieldtype] = strtok(tmp2);
	fieldname = genvarname(fieldname);
	
	% skip LOOKUP_TABLE
	str = fgetl(fid);

	if (~strcmp(lower(info.DatasetFormat),'binary'))
		% TODO use type
	        [dat,n_]  = fscanf(fid,'%f');
	else
        ...
        % dat  = reshape(dat,n,1);
	mesh.cell_data.(fieldname) = dat;
	end
    elseif (strfind(str,'points'))
	...
	% p_ = reshape(num,3,npoints);
	else
		p = fread(fid,[3,npoints],'double','ieee-be')'; 
	end
        mesh.points = p;
        clear p;
	...
	if (~strcmp(lower(info.DatasetFormat),'binary'))
		...
		t = fread(fid,[5,ncells],'uint32','ieee-be')';
		t = t(:,2:end);
	end
        mesh.cells = t+1; % in matlab, first index is 1;
        mesh.ncells = size(t,1);
        clear t;
	%fpos = ftell(fid)
	%fseek(fid, 0, 'eof');
	%l = ftell(fid)
